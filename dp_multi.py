...
p, n = np.reshape(window_formation(p),(-1,input_size,2)), np.reshape(window_formation(n),(-1,input_size,2))

labels = []
processed_features = scale(np.diff(data[:,0]),with_std=True)
